...
export = function slugify(input: string, separator: string = '-'): string {
    ...
    const slug = input.split('')
        ...
        .replace(none, '')
        .replace(glue, separator);
    const from = slug.startsWith(separator) ? 1 : 0;
    const back = slug.endsWith(separator) ? 1 : 0;
    if (from + back === 0) {
        return slug;
    } else {
        return slug.substring(from, slug.length - back);
    }
};
